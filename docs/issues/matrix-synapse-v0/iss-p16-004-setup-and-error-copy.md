# ISS-P16-004: Setup and error copy

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P16-004: Setup and error copy
- **Suggested branch:** `issue/iss-p16-004-setup-and-error-copy`
- **Suggested PR title:** `ISS-P16-004: Setup and error copy`
- **Primary repo:** `ZenithResearch/ZenithOS`
- **Supporting repo/API dependency:** `ZenithResearch/hub`
- **Source vault note:** `private source note: iss-p16-004`
- **GitHub issue:** https://github.com/ZenithResearch/ZenithOS/issues/4
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p16-004-setup-and-error-copy.md`

## Full spec

### Objective

Add clear localhost-vs-production setup copy and visible login/register/DM/send failure states.
...
- PRP-PR-014/015 decisions for production copy

### Target files and surfaces

- MatrixClient.swift, MatrixLoginView.swift, MatrixInboxView.swift, HubConfigView.swift
...
- Network/DNS/TLS/auth/send failures surface without leaking sensitive responses; registration-disabled behavior is clear.
...
- swift test; swift build; manual invalid homeserver/password/send smoke.
...
**Commit boundary:** one commit in the `ISS-P16-004` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P16-004`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p16-004"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p16-004 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p16-004"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p16-004 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p16-004 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p16-004 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p16-004`.
