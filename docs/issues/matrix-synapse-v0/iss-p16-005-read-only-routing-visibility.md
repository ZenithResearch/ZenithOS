# ISS-P16-005: Read-only routing visibility

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P16-005: Read-only routing visibility
- **Suggested branch:** `issue/iss-p16-005-read-only-routing-visibility`
- **Suggested PR title:** `ISS-P16-005: Read-only routing visibility`
- **Primary repo:** `ZenithResearch/ZenithOS`
- **Supporting repo/API dependency:** `ZenithResearch/hub`
- **Source vault note:** `private source note: iss-p16-005`
- **GitHub issue:** https://github.com/ZenithResearch/ZenithOS/issues/5
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p16-005-read-only-routing-visibility.md`

## Full spec

### Objective

Show passive/mentions-only/concierge/agent-visible routing state as read-only until Hub supports safe config writes.
...
- Hub read-only routing status from PRP-PR-018

### Target files and surfaces

- MatrixInboxView.swift, HubConfigView.swift, SynapseInboxView.swift, future routing-status consumer
...
- Operator can distinguish passive/private rooms from queue/case-producing rooms; no mutation toggles exist before Hub write API.
...
- swift test; swift build; mocked routing-mode UI smoke.
...
**Commit boundary:** one commit in the `ISS-P16-005` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P16-005`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p16-005"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p16-005 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p16-005"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p16-005 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p16-005 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p16-005 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p16-005`.
