# ISS-P16-002: Matrix readiness panel

> Issue = PR boundary. Tasks below = commit boundaries inside that PR.

## PR boundary

- **PR scope:** ISS-P16-002: Matrix readiness panel
- **Suggested branch:** `issue/iss-p16-002-matrix-readiness-panel`
- **Suggested PR title:** `ISS-P16-002: Matrix readiness panel`
- **Primary repo:** `ZenithResearch/ZenithOS`
- **Supporting repo/API dependency:** `ZenithResearch/hub`
- **Source vault note:** `private source note: iss-p16-002`
- **GitHub issue:** https://github.com/ZenithResearch/ZenithOS/issues/2
- **Repo-local spec path:** `docs/issues/matrix-synapse-v0/iss-p16-002-matrix-readiness-panel.md`

## Full spec

### Objective

Display Hub Matrix/Synapse readiness in ZenithOS from a redacted Gateway admin status endpoint.
...
- PRP-PR-015 readiness endpoint

### Target files and surfaces

- Sources/ZenithOSUI/Hub/HubConfigView.swift, ReviewAccessHubClient.swift, Matrix/, SynapseInboxView.swift
...
- Panel shows homeserver URL, reachability, configured booleans, and service identity status using redacted data only.
...
- swift test; swift build; mocked status payload UI smoke.
...
**Commit boundary:** one commit in the `ISS-P16-002` PR.

**Objective:** Read the source vault note and inspect the target repo surfaces for `ISS-P16-002`. Confirm the exact files/modules to touch, record current behavior, and update this spec if discovery changes the file list.
...
git commit -m "docs: scope iss-p16-002"
```
...
```bash
git add <changed-files>
git commit -m "test: cover iss-p16-002 contract"
```
...
```bash
git add <changed-files>
git commit -m "feat: implement iss-p16-002"
```
...
```bash
git add <changed-files>
git commit -m "test: harden iss-p16-002 edge cases"
```
...
```bash
git add <changed-files>
git commit -m "docs: record iss-p16-002 operator evidence"
```
...
```bash
git add <changed-files>
git commit -m "chore: verify iss-p16-002 pr readiness"
```
...
- [ ] Link to source vault note `private source note: iss-p16-002`.
